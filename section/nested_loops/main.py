...
while i < len(travel_costs):           # outer loop over trips
    j = 0
    max_cost = travel_costs[i][0]      # start with the first expense

    while j < len(travel_costs[i]):    # inner loop over expenses
        if travel_costs[i][j] > max_cost:
            max_cost = travel_costs[i][j]
        j += 1
    max_costs.append(max_cost)         # store the highest expense found
    i += 1
